...
// ─────────────────────────────────────────────────────────────────
//  CROP UTILITY — FIXED iOS + ANDROID  (UNCHANGED)
//
//  iOS:
...
//  CACHE STORAGE — NEW
//
//  Copies the cropped 512x512 image into the app cache directory
//  under /eye-captures/, and writes a small JSON pointer file
//  (`<eyeSide>_latest.json`) recording the most recent capture for
//  that eye so it can be retrieved later without navigating away
//  from this screen.
// ─────────────────────────────────────────────────────────────────
async function storeImageInCache(
  croppedUri: string,
  eyeSide: "left" | "right",
): Promise<string> {
  try {
    const destDir = `${FileSystem.cacheDirectory}eye-captures/`;
    ...
    const filename = `${eyeSide}_${timestamp}.jpg`;
    const destUri = destDir + filename;

    await FileSystem.copyAsync({ from: croppedUri, to: destUri });

    // Keep a small metadata pointer so the latest capture per eye
    // can be retrieved later without leaving this screen.
    const cacheImage = { uri: destUri, timestamp, eyeSide };
    await FileSystem.writeAsStringAsync(
      `${destDir}${eyeSide}_latest.json`,
      JSON.stringify(cacheImage),
    );
    ...
    // Fall back to the original cropped uri so the popup can still show something.
    return croppedUri;
  }
}
...
export default function EyeCaptureScreen() {
  ...
  // ── NEW: post-capture popup / pause state ──────────────────────
  ...
  // ── AUTO-CAPTURE ──────────────────────────────────────────────
  // Guarded against firing while the camera is paused or the
  // success popup is open, so a held "all good" condition can never
  // produce a duplicate capture once the popup is showing.
  useEffect(() => {
    if (
      eyeConfirmed &&
      qualityGood &&
      !capturing &&
      !hasFiredCapture.current &&
      !cameraPaused &&
      !showCapturePopup
    ) {
  ...
  }, [eyeConfirmed, qualityGood, capturing, cameraPaused, showCapturePopup]);

  // ── Capture handler ───────────────────────────────────────────
  // NOTE: this no longer navigates anywhere. On success it stores the
  // cropped 512x512 image in cache and opens the success popup.
  const handleCapture = useCallback(async () => {
    ...
    try {
      ...
      // Crop to 512x512 — existing utility, unchanged.
      const croppedUri = await cropGuideBoxTo512(uri, currentDims);

      // Store the cropped 512x512 image in the app cache directory.
      const cachedUri = await storeImageInCache(croppedUri, eyeSide);

      clog("cache-store-success", {
        uri: cachedUri,
        eyeSide,
        timestamp: Date.now(),
      });
      ...
      // No navigation. Show the success popup and pause the camera
      // until the user dismisses it.
      setCachedImageUri(cachedUri);
      setCameraPaused(true);
      setShowCapturePopup(true);
      clog("capture-popup-open", { uri: cachedUri });
    } catch (err: any) {
      const totalMs = Date.now() - captureStart;
      clog("capture-error", {
        eyeSide,
  ...
  }, [capturing, eyeSide, quality, facing]);

  // ── Popup dismiss handler ────────────────────────────────────
  // Resets all detection state and restarts the camera so the user
  // can capture another image without leaving this screen.
  const handleClosePopup = useCallback(() => {
    clog("capture-popup-close", { restartCamera: true });

    setShowCapturePopup(false);

    // Reset detection state for a fresh session.
    setEyeConfirmed(false);
    setDetectedBox(null);
    setCapturing(false);
    hasFiredCapture.current = false;
    ...
    prevProblemKey.current = null;
    lastSpokenKey.current = null;
    lastSpokenTime.current = 0;

    // Restart camera + frame processing.
    setCameraPaused(false);
    clog("camera-restarted", { reason: "popup-dismissed" });
  }, []);

  // ── Frame processor ───────────────────────────────────────────
  // UNCHANGED — identical detection / quality / inference logic.
  ...
  return (
    <View style={s.root}>
      <Camera
        ...
        frameProcessor={cameraPaused ? undefined : frameProcessor}
        photo={true}
        onInitialized={() => {
      ...
      <View style={s.captureArea}>
        ...
        <TouchableOpacity
          ...
          disabled={capturing || cameraPaused}
          activeOpacity={0.75}
        >
          {capturing ? (
            <ActivityIndicator color="#fff" />
      ...
      {/* ── SUCCESS POPUP — shown after capture, blocks no navigation ── */}
      <Modal
      ...
      >
        <View style={s.popupOverlay}>
          <View style={s.popupCard}>
            <View style={s.popupIconCircle}>
              <Text style={s.popupIconCheck}>✓</Text>
            </View>
            <Text style={s.popupTitle}>Image Captured</Text>
            <Text style={s.popupMessage}>
              512×512 eye image successfully captured and stored in cache.
            </Text>
            <TouchableOpacity style={s.popupOkBtn} onPress={handleClosePopup}>
              <Text style={s.popupOkBtnText}>OK</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
const s = StyleSheet.create({
  ...
  popupIconCheck: { color: "#00e676", fontSize: 28, fontWeight: "700" },
  popupTitle: {
    color: "#fff",
    fontSize: 18,
    fontWeight: "700",
  ...
  popupOkBtnText: { color: "#fff", fontWeight: "700", fontSize: 16 },
} as any);